...
def main():
    ...
    pressure_path = pjoin(args.data_path, "pressure", args.file_name + ".npz")
    text_path = pjoin(args.data_path, "texts", args.file_name + ".txt")
    ...
    n_frames = pressure.shape[0] - 1
    is_using_data = not any([args.text_prompt])
    is_using_data = False
    dist_util.setup_dist(args.device)
    if out_path == '':
        out_path = os.path.join(os.path.dirname(args.model_path),"results_selected",
                                'samples_{}_seed{}'.format(args.file_name, args.seed))
        if args.text_prompt != '':
    ...
    if is_using_data:
        ...
        collate_args = [{'inp': torch.zeros(n_frames), 'tokens': None, 'lengths': n_frames, 'pressure': pressure}] * args.num_samples
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            plot_3d_motion(animation_save_path, skeleton, motion, dataset=args.dataset, title=args.file_name, fps=fps, hint=hint)
            # plot_3d_motion(animation_save_path, skeleton, motion, title=args.file_name, fps=fps)
