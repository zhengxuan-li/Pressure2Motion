...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.mean = torch.from_numpy(np.load(pjoin('./datasets', 'mean_global.npy'))).float()
        self.std = torch.from_numpy(np.load(pjoin('./datasets', 'std_global.npy'))).float()
    ...
    def consistency_loss(self, pred_joints, target_joints, mask=None):
        # Compute the consistency loss between the predicted and target joint positions
        # Assuming pred_joints and target_joints are computed from model output and ground truth respectively
        # and have the shape [B, seq_len, 22, 3]

        contact_joint_indices = [0, 7, 8, 10, 11]
        pred_contact_joints = pred_joints[:, :, contact_joint_indices, :]
        target_contact_joints = target_joints[:, :, contact_joint_indices, :]

        pred_joints = pred_contact_joints.reshape(
            pred_contact_joints.shape[0], pred_contact_joints.shape[1], -1
        )
        target_joints = target_contact_joints.reshape(
            target_contact_joints.shape[0], target_contact_joints.shape[1], -1
        ) # B, seq_len, 15
        pred_joints = pred_joints.permute(0, 2, 1).unsqueeze(2)  # B, 15, 1, seq_len
        target_joints = target_joints.permute(0, 2, 1).unsqueeze(2)  # B, 15, 1, seq_len
        if mask is not None:
            return self.masked_l2(pred_joints, target_joints, mask)
        return self.l2_loss(pred_joints, target_joints)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        """
        enc = model.model
        mask = model_kwargs['y']['mask'] # b, 1, 1, 160
        joints = model_kwargs['y']['joints'].clone().detach() # b, 160, 22, 3

        if self.raw_std.device != joints.device:
            self.raw_mean = self.raw_mean.to(joints.device)
            self.raw_std = self.raw_std.to(joints.device)
            self.mean = self.mean.to(joints.device)
            self.std = self.std.to(joints.device)
        joints = joints.view(joints.shape[0], joints.shape[1], -1)  # [bs, njoints, 66]
        joints = joints * self.raw_std + self.raw_mean
        joints = joints.view(joints.shape[0], joints.shape[1], 22, 3)  # [bs, njoints, 22, 3]
        ...
        x_ = model_output.permute(0, 3, 2, 1).contiguous() # [bs, 160,, 1, 263]
        x_ = x_.squeeze(2) # [bs, 160, 263]
        x_ = x_ * self.std + self.mean
        n_joints = 22 if x_.shape[-1] == 263 else 21
        joint_pos = recover_from_ric(x_, n_joints)
        
        terms["consistency_loss"] = self.consistency_loss(
            pred_joints=joint_pos,
            target_joints=joints,
            mask=mask
        )
        terms["loss"] = terms["rot_mse"] +  5 * terms["consistency_loss"]
