...
def calc_MPJPE(joint, gt_joint):
    """
    Calculate Mean Per Joint Position Error (MPJPE).
    :param joint: predicted joint positions, shape [seq, 22, 3]
    :param gt_joint: ground truth joint positions, shape [seq, 22, 3]
    :return: MPJPE value, a scalar.
    """
    # Calculate Euclidean distance for each joint at each frame
    # error shape: [seq, 22]
    error = np.linalg.norm(joint - gt_joint, axis=-1)
    # Return the mean error, a scalar
    return np.mean(error)

def calc_MPJPE_lower(joint, gt_joint):
    """
    Calculate Mean Per Joint Position Error (MPJPE) for the lower body.
    :param joint: predicted joint positions, shape [seq, 22, 3]
    :param gt_joint: ground truth joint positions, shape [seq, 22, 3]
    :return: Lower body MPJPE value, a scalar.
    """
    # Indices for lower body joints in HumanML3D skeleton
    # (pelvis, l_hip, r_hip, l_knee, r_knee, l_ankle, r_ankle, l_foot, r_foot)
    lower_body_indices = [0, 1, 2, 4, 5, 7, 8, 10, 11]
    
    # Select only the lower body joints
    joint_lower = joint[:, lower_body_indices, :]
    gt_joint_lower = gt_joint[:, lower_body_indices, :]
    
    # Calculate Euclidean distance for each lower body joint at each frame
    # error shape: [seq, 9]
    error = np.linalg.norm(joint_lower - gt_joint_lower, axis=-1)
    # Return the mean error, a scalar
    return np.mean(error)
