...
def get_opt(opt_path, device):
    ...
    opt.data_root = './datasets/MPL'
    opt.joints_dir = pjoin(opt.data_root, 'joints')
    opt.pressure_dir = pjoin(opt.data_root, 'pressure')
    opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs_global')
    opt.text_dir = pjoin(opt.data_root, 'texts')

    opt.joints_num = 22
    opt.dim_pose = 263
    opt.max_motion_length = 160
