...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __init__(self, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                print('Processing batch %d/%d' % (i, real_num_batches))
                ...
                for t in range(repeat_times):
                    ...
                    # noise = th.randn_like(motion, device=dist_util.dev())
                    # timesteps =  torch.zeros(motion.shape[0], device=dist_util.dev(), dtype=torch.long)
                    # with torch.no_grad():
                    #     sample = model(noise, timesteps, **model_kwargs)

                    if t == 0:
                        sub_dicts = [{'motion': sample[bs_i].squeeze().permute(1,0).cpu().numpy(),
                                    'length': model_kwargs['y']['lengths'][bs_i].cpu().numpy(),
                                    'caption': model_kwargs['y']['text'][bs_i],
                                    'feet_hint': model_kwargs['y']['feet_hint'][bs_i].cpu().numpy() if 'feet_hint' in model_kwargs['y'] else None,
                                    'joints': model_kwargs['y']['joints'][bs_i].cpu().numpy() if 'joints' in model_kwargs['y'] else None,
                                    'tokens': tokens[bs_i],
                                    'cap_len': len(tokens[bs_i]),
                                    'pressure': model_kwargs['y']['pressure'][bs_i].cpu().numpy() if 'pressure' in model_kwargs['y'] else None,
    ...
    def __getitem__(self, item):
        ...
        motion, m_length, caption, tokens, pressure, feet_hint, joints = data['motion'], data['length'], data['caption'], data['tokens'], data['pressure'], data['feet_hint'], data['joints']
