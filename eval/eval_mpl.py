        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                if len(batch) == 7:
                    ...
                    word_embeddings, pos_one_hots, _, sent_lens, pressure, motions, m_lens, _ , _, _ = batch
...
def evaluate_control(motion_loaders, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(motion_loader):
            word_embeddings, pos_one_hots, _, sent_lens, pressure, motions, m_lens, _ , hint, joints = batch
            # process motion
            # sample to motion
            ...
            joints = joints * raw_std + raw_mean
            joints = joints.reshape(joints.shape[0], joints.shape[1], n_joints, 3)
            if n_joints == 21:
                hint = hint * 0.001
            hint = hint.view(hint.shape[0], hint.shape[1], n_joints, 3) * mask_hint
            for motion, h, mask, joint in zip(motions, hint, mask_hint, joints):
                ...
                mpjpe_sum += calc_MPJPE(motion.numpy(), joint.numpy())
                lmpjpe_sum += calc_MPJPE_lower(motion.numpy(), joint.numpy())
        ...
        # MPJPE
        mpjpe_score = mpjpe_sum / all_size
        MPJPE[motion_loader_name] = mpjpe_score

        lmpjpe_score = lmpjpe_sum / all_size
        LMPJPE[motion_loader_name] = lmpjpe_score
    ...
    print(f'---> [{motion_loader_name}] MPJPE: {mpjpe_score:.4f}')
    print(f'---> [{motion_loader_name}] MPJPE: {mpjpe_score:.4f}', file=file, flush=True)
    print(f'---> [{motion_loader_name}] LMPJPE: {lmpjpe_score:.4f}')
    print(f'---> [{motion_loader_name}] LMPJPE: {lmpjpe_score:.4f}', file=file, flush=True)
    ...
    print(line)
    print(line, file=file, flush=True)
    return l2_dict, skating_ratio_dict, trajectory_score_dict, MPJPE, LMPJPE
...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            _, _, _, sent_lens, pressure, motions, m_lens, _ , _ ,_ = batch
...
def evaluation(eval_wrapper, gt_loader, eval_motion_loaders, log_file, replication_times, diversity_times, mm_num_times, run_mm=False):
    with open(log_file, 'w') as f:
        ...
        for replication in range(replication_times):
            ...
            print(f'Time: {datetime.now()}')
            print(f'Time: {datetime.now()}', file=f, flush=True)
            control_l2_dict, skating_ratio_dict, trajectory_score_dict, MPJPE, LMPJPE = evaluate_control(motion_loaders, f)
            ...
            for key, item in MPJPE.items():
                if key not in all_metrics['MPJPE']:
                    all_metrics['MPJPE'][key] = [item]
                else:
                    all_metrics['MPJPE'][key] += [item]

            for key, item in LMPJPE.items():
                if key not in all_metrics['LMPJPE']:
                    all_metrics['LMPJPE'][key] = [item]
                else:
                    all_metrics['LMPJPE'][key] += [item]
...
if __name__ == '__main__':
    ...
    log_file = os.path.join(os.path.dirname(args.model_path), 'eval_mpl_{}_{}'.format(name, niter))
    ...
    gen_loader = get_dataset_loader(args, name=args.dataset, batch_size=args.batch_size, num_frames=None, split=split, hml_mode='eval')
