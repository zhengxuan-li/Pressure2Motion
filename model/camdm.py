...
class CAMDM(torch.nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        # ------
        # --- CAMDM ---
        self.input_cond_block = GlobalBlock(self.data_rep, 39, self.latent_dim)
        ...
        adapterLayer = AdapterLayer(d_model=self.latent_dim,
                                     nhead=self.num_heads,
                                     dim_feedforward=self.ff_size,
                                     dropout=self.dropout,
                                     activation=self.activation)
        self.Adapter = Adapter(adapterLayer, self.num_layers)
        self.adapter_embed_text = nn.Linear(self.clip_dim, self.latent_dim)
    ...
    def camdm_forward(self, x, timesteps, y=None, weight=1.0):
        """
        Realism Guidance
        ...
        guided_pressure = self.input_cond_block(y['pressure'])
        ...
        text_embed = self.adapter_embed_text(self.mask_cond(enc_text, force_mask=force_mask)).unsqueeze(0)  # [1, bs, d]
        control = self.Adapter(control, text_embed, y['pressure'])
        return control
    ...
    def forward(self, x, timesteps, y=None):
        ...
        timesteps: [batch_size] (int)
        """
        control = self.camdm_forward(x, timesteps, y)
...
class GlobalBlock(nn.Module):
    def __init__(self, data_rep, input_feats, latent_dim):
        ...
        self.pressure_stage1 = Pressure()

    def forward(self, x):
        # x: pressure
        x1 = self.pressure_stage1(x)[:,:-1,:]
        x = x1.permute((1, 0, 2))
